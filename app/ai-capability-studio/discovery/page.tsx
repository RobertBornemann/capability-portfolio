...
type DemoResponse = { run_id: string; steps: string[]; insights: any; variant: Variant; cached?: boolean };
...
export default function DiscoveryDemo() {
  ...
  const [samples, setSamples] = useState<{ sensitive: string; sanitized: string }>({ sensitive: "", sanitized: "" });
  ...
  return (
    <main className="mx-auto max-w-5xl px-4 py-10">
      ...
      <div className="mt-4 flex items-center gap-3">
        <Radio checked={variant === "sanitized"} onChange={() => setVariant("sanitized")}>Sanitized transcript</Radio>
        <Radio checked={variant === "sensitive"} onChange={() => setVariant("sensitive")}>Sensitive transcript (demo)</Radio>
      </div>
      ...
      {steps.length > 0 && (
        <div className="mt-5 flex flex-wrap gap-2">
          {steps.map((s, i) => (
            <span key={i} className="rounded-full border px-3 py-1 text-xs">{s}</span>
          ))}
          {!loading && <span className="rounded-full border px-3 py-1 text-xs">Done</span>}
        </div>
      )}
      ...
      <div className="inline-flex rounded-full border p-1">
        {tabs.map(t => (
          <button
            ...
            className={`rounded-full px-4 py-1 text-sm ${value === t.id ? "bg-black text-white" : ""}`}
          >
            {t.label}
          </button>
        ))}
...
function JsonPanel({ data }: { data: unknown }) {
  ...
  function download() {
    ...
    a.href = url; a.download = "insights.json"; a.click();
    URL.revokeObjectURL(url);
  }
  return (
    <div className="mt-4">
      <div className="mb-2 flex gap-2">
        <button onClick={copy} className="rounded border px-3 py-1 text-xs">Copy</button>
        <button onClick={download} className="rounded border px-3 py-1 text-xs">Download</button>
      </div>
      <pre className="max-h-[28rem] overflow-auto rounded-2xl border bg-neutral-50 p-4 text-xs">{pretty}</pre>
    </div>
  );
}
...
function InsightsGrid({ insights }: { insights: any }) {
  return (
    <section className="mt-4 space-y-8">
      <InsightSection title="Pain Points" items={insights.pain_points} render={(pp: any) => (
        <InsightCard title={pp.description} lines={[`Impact: ${pp.impact}`, pp.quote ? `“${pp.quote}”` : undefined]} />
      )} />
      <InsightSection title="Jobs-to-be-Done" items={insights.jobs_to_be_done} render={(j: any) => (
        <InsightCard title={j.functional_job} lines={[`Emotional: ${j.emotional_job}`, `Context: ${j.context}`]} />
      )} />
      <InsightSection title="Workarounds" items={insights.workarounds} render={(w: any) => (
        <InsightCard title={w.what_they_do} lines={[`Why: ${w.why_needed}`, `Cost: ${w.cost}`]} />
      )} />
      <InsightSection title="Desired Outcomes" items={insights.desired_outcomes} render={(o: any) => (
        <InsightCard title={o.outcome} lines={[`Gap: ${o.current_gap}`]} />
      )} />
      <InsightSection title="Behavioral Signals" items={insights.behavioral_signals} render={(b: any) => (
        <InsightCard title={b.observation} lines={[`Reveals: ${b.what_it_reveals}`]} />
      )} />
      <InsightSection title="Mental Models" items={insights.mental_models} render={(m: any) => (
        <InsightCard
          title={m.description}
          lines={[
...
function InsightSection({
  ...
  return (
    <div>
      <h2 className="text-lg font-semibold">{title} <span className="text-neutral-500 text-sm">({items.length})</span></h2>
      <div className="mt-3 grid gap-3">{items.map((it, i) => <div key={i}>{render(it)}</div>)}</div>
    </div>
  );
}
...
function InsightCard({ title, lines }: { title: string; lines?: (string | undefined)[] }) {
  return (
    <div className="rounded-2xl border p-4">
      ...
      {lines?.map((l, i) => l && (<div key={i} className="text-sm text-neutral-600">{l}</div>))}
    </div>
  );
}
  ...
  return (
    <div className="mt-4 grid gap-4 md:grid-cols-2">
      <InfoBox title="What this is">
        AI workflow tool for automated analysis of user interviews. 
        It identifies pain points, jobs to be done, workarounds, desired outcomes, and mental models by applying predefined 
        research configurations. 
        The system produces outputs ready for Miro or Mural and is built with sensible defaults, 
        embedded guardrails, and strong security to ensure inputs are sanitized and sensitive 
        data safely removed.
      </InfoBox>
      ...
      <InfoBox title="Architecture">
        <ul className="list-disc space-y-1 pl-5 text-sm text-neutral-700">
          ...
          <li>Session limits and variant-scoped caching<code>/tmp</code>.</li>
        </ul>
      </InfoBox>
...
agent = Agent(
  ...
  model_settings={"temperature": 0.3},
      ...
      <InfoBox title="Who uses this">
        <ul className="list-disc space-y-1 pl-5 text-sm text-neutral-700">
          <li><b>PMs & UX:</b> faster synthesis, consistent artifacts.</li>
          <li><b>Platform teams:</b> analyse & compare interviews without support.</li>
          <li><b>Leads:</b> privacy, governance, and cost control by design.</li>
        </ul>
      </InfoBox>
    </div>
  );
...
function CodeBlock({ children }: { children: string }) {
  ...
  useEffect(() => { ref.current?.scrollTo({ top: 0 }); }, [children]);
  ...
  return (
    <div>
      ...
      <pre ref={ref} className="max-h-48 overflow-auto rounded-2xl border bg-neutral-50 p-4 text-xs">
        {children}
      </pre>
    </div>
  );
